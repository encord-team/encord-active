...
class VideoRedMetric(VideoColorMetric):
    def __init__(self):
        super(VideoRedMetric, self).__init__("Red")


class VideoGreenMetric(VideoColorMetric):
    def __init__(self):
        super(VideoGreenMetric, self).__init__("Green")


class VideoBlueMetric(VideoColorMetric):
    def __init__(self):
...
if __name__ == "__main__":
    ...
    path = sys.argv[1]
    execute_metrics([VideoBlueMetric()], data_dir=Path(path))
    execute_metrics([VideoGreenMetric()], data_dir=Path(path))
    execute_metrics([VideoRedMetric()], data_dir=Path(path))
