...
from encord_active.db.models import Project, ProjectDataMetadata, ProjectDataUnitMetadata, ProjectPrediction
...
def import_project(engine: Engine, database_dir: Path, project: ProjectImportSpec) -> None:
    ...
    for data in project.project_data_list:
        if data.project_hash != project_hash:
            raise ValueError(f"Project import, project hash inconsistency")
