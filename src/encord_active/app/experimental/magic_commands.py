...
# Merged metrics column name
image_id = "image_id"
data_type = "data_type"
object_id = "object_id"

# Classification predictions column name
correct_prediction = "prediction"
ground_truth_class_name = "ground truth class name"
...
Return the python code (do not import anything) and make sure to prefix the requested python code with 
...
instruct_message_classification_performance = """Today is {today_date}.
You are provided with a pandas dataframe (df) with {df_num_rows} rows and {df_num_cols} columns.
This is the result of `print(df.head())`:
{df_head}.
Each row corresponds to the properties of a single image. `{correct_prediction}` column indicates if the class prediction for 
the image is true (1) or false (0) prediction. `{ground_truth_class_name}` indicates the label of the image, and `{predicted_class_name}` 
indicates predicted class.
...
error_correction_message_classification_performance = """
Today is {today_date}.
You are provided with a pandas dataframe (df) with {df_num_rows} rows and {df_num_cols} columns.
This is the result of `print(df.head())`:
{df_head}.
Each row corresponds to the properties of a single image. `{correct_prediction}` column indicates if the prediction of
that column is true (1) or false (0). `{ground_truth_class_name}` indicates the label of the image, and `{predicted_class_name}` 
indicates predicted class name.
...
def magic_commands():
    def _get_chatgpt_response(
        instruct: str, question: str, start_code_tag: str, end_code_tag: str, temperature: Optional[float] = None
    ) -> str:
    ...
    def render():
        ...
        with model_performance_tab:
            ...
            if classification_performance_builder.is_available():
                classification_performance_builder.load_data(ModelQualityPage.METRICS)
                df = classification_performance_builder._model_predictions.copy()

                # Make it a bit more verbose for the LLM
                df = df.rename(
                    columns={
                        "class_name": predicted_class_name,
                        "gt_class_name": ground_truth_class_name,
                ...
                if magic_prompt != "":
                    ...
                    while count < TOTAL_TRIAL:
                        try:
                            ...
                            count += 1
                            code_to_run = _get_chatgpt_response(
                                instruct=error_correction_message_classification_performance.format(
                                    today_date=date.today(),
                                    df_num_rows=df.shape[0],
                                    df_num_cols=df.shape[1],
                                    df_head=df.head(),
                    ...
                    if valid_code:
                        st.write(f"Generated code for this prompt (in {count + 1}. attempt):")
                        st.code(code_to_run, language="python", line_numbers=True)
                        ...
                        st.write(f"Tried {count} times, however a code could not be generated for this prompt :(")
                else:
                    st.write("please provide a prompt to see the result")
