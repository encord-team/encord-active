...
If your predictions are stored in the [COCO results format](https://cocodataset.org/#format-results) you can jump to the [import coco predictions](#importing-coco-predictions) section.
...
Note that for `image_groups`, each image in the group has its own `data_hash`, while
...
You should specify masks as binary `numpy` arrays of size [height, width] and with `dtype` `np.uint8`.
...
:::note
This command assume that you have imported you project using the [COCO importer](../cli#project) and you are inside the projects directory.
:::
...
After the execution is done, you should be ready to [evaluate your model performance](../workflows/evaluate-detection-model).
