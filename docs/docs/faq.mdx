...
You can, for example, use the [`initialize`](./cli/initialising-project-from-image-directories) command to initialize a project from an image directory.
...
1. As soon as the metric metric computations have started (indicated by Encord Active printing a line containing `Running metric`) you can open a new terminal and run `encord-active visualize`.
   ...
   Do a browser refresh once in a while when new metrics are done computing in your first terminal.
...
2. Finding label errors that potentially harm your model performance
...
Uploading your data to Encord Annotate is as simple as clicking the _Clone_ button on the [Filter and Export][filter-and-export] page.
