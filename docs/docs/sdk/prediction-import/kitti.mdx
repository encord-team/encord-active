...
It consists of a collection of high-resolution images, lidar data, and camera calibration parameters captured from a moving platform driving through urban environments.
The dataset includes a variety of tasks such as object detection, tracking, 3D object detection, and semantic segmentation.

:::caution

The following approach **only works for bounding boxes**.

Every time you run this importer, previously imported predictions will be overwritten!
Make sure to [version your project][project-versioning] if you want to be able to go back to previous model iterations.
...
The KITTI importer supports the label format described [here](https://docs.nvidia.com/tao/tao-toolkit-archive/tlt-20/tlt-user-guide/text/preparing_data_input.html#label-files) with the addition of a column corresponding to the model confidence.
...
project_path = Path("/path/to/your/project/folder")
...
print({o["featureNodeHash"]: o["name"] for o in encord_project.ontology["objects"]})
# Example output:
...
Once you have a valid KITTI predictions folder, you can import the predictions running the following script:
...
predictions_root = Path("/path/to/your/predictions/folder")
