...
This document takes you through how to initialize a project **with code**.
...
## Initialize a Project with Labels
...
A label transformer will take as input a list of label files and a list of data files (the files we provide to the `init_local_project` below) and need to return a list of `DataLabel`s.
...
Once the project is initialized, consider executing some metrics on your project.
