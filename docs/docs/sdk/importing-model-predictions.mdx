...
```python
img_h, img_w = 720, 1280  # the image size in pixels
polygon = polygon / np.array([[img_h, img_w]])
```

Notice the double braces `[[img_h, img_w]]` to get an array of shape `[1, 2]`.
...
```shell
(ea-venv)$ encord-active visualise /path/to/your/data
```
