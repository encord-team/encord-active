...
In addition to the above, a **classification** prediction contains 3 identifiers, `classification_hash` `attribute_hash` and `option_hash` while an **object detection** prediction contains the `object_class_hash`, the actual prediction `data`, and the `format` of that data.
...
:::tip
To find the 3 hashes we can inspect the ontology by running
