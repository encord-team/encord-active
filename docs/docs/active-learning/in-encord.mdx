...
If you don't have any projects yet, you can watch the tutorial video on setting up a [workflow project](https://docs.encord.com/projects/workflows/creating-and-configuring-workflows#workflows---tutorial) to get started quickly.
...
2. Select the `No class` option under `Object Class` to filter unannotated data.
...
1. In the _Filter_ tab, select the `No class` option under `Object Class` to filter unannotated data.
2. Use the [bulk tagging][bulk-tagging-feature] feature to mark them with a data tag, such as `unlabeled`.
3. Access the _Action tab_ in the toolbox.
4. Click on the <kbd>✅ Mark as Complete</kbd> button and follow the instructions to temporarily move all unannotated data to the workflow's `Complete` stage.
5. Return to the _Filter_ tab, add the `Image Difficulty` filter and adjust the slider to select the 5% of unannotated data with the lowest score.
6. Access the _Action_ tab in the toolbox again.
7. Click on the <kbd>🖋 Relabel</kbd> button and follow the instructions to [prioritize labeling][relabeling-guide] for the selected data.
8. Once the selected data has been labeled, use the following filter combination to bring back the unannotated data from to the `Complete` stage to the first labeling stage:
  Select the  `No class` option under `Object Class` and choose the proper tag name (e.g. `unlabeled`) option under `Tag`.
